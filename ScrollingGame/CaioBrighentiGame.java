...
public class CaioBrighentiGame {
	...
	private static final int FACTOR = 2;      // you might change that this
	...
	public void addPiece() {
		activePiece = new Piece(new Location(2,9), Color.BLACK);
		grid.setCellImage(activePiece.getLoc(), TEST_IMG);	
	}
	...
	public void checkCollumns(){
		// Iterate through each col
		for (int col = 0; col < grid.getNumCols(); col++) {
			// For each col, check if it is full and if so clear it
			if (checkFullCol(col))
				clearCol(col);
		}
	}

	public boolean checkFullCol(int colnumber){
		for (int row = 0; row < grid.getNumRows(); row++) {
			if (grid.getCellImage(new Location(row, colnumber)) != TEST_IMG)
				return false;
		}
		return true;
	}
	...
	public void clearCol(int colnumber){
		for (int row = 0; row < grid.getNumRows(); row++) {
			grid.setCellImage(new Location(row, colnumber), null);
		}
	}
